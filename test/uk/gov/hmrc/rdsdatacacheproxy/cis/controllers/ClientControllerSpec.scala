...
class ClientControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ClientController#getClientsByEmployersReference" - {
    ...
    "returns 200 with client list when valid parameters provided" in new Setup {
      ...
      verify(mockService).getClientsByEmployersReference(
        eqTo("IR123456"),
        eqTo("CRED-ABC-123"),
        eqTo("123456")
      )
    ...
    "returns 400 when irAgentId is empty" in new Setup {
      val req = FakeRequest(GET, "/client-empRef?irAgentId=&credentialId=CRED-ABC-123&employerRef=123456")
    ...
    "returns 400 when credentialId is empty" in new Setup {
      val req = FakeRequest(GET, "/client-empRef?irAgentId=IR123456&credentialId=&employerRef=123456")
    ...
    "returns 400 when employerRef contains only whitespace" in new Setup {
      val req = FakeRequest(GET, "/client-empRef?irAgentId=IR123456&credentialId=CRED-ABC-123&employerRef=%20%20%20")
      val res: Future[Result] = controller.getClientsByEmployersReference("IR123456", "CRED-ABC-123", "")(req)
