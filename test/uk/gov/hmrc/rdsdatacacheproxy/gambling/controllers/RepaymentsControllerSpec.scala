...
class RepaymentsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "RepaymentsController#getRepaymentsSummary" - {
    ...
    "returns 200 when service succeeds" in new Setup {
      ...
      when(mockService.getRepaymentsSummary(eqTo(validRegime), eqTo("XWM00000001770"))(any()))
        .thenReturn(Future.successful(Right(validResponseRepaymentsSummary)))

      val req = FakeRequest(GET, s"/gambling/repayment-summary/$validRegime/XWM00000001770")
      val res: Future[Result] = controller.getRepaymentsSummary(validRegime, "XWM00000001770")(req)

      status(res) mustBe OK
      contentType(res) mustBe Some(JSON)
      contentAsJson(res) mustBe Json.toJson(validResponseRepaymentsSummary)

      verify(mockService).getRepaymentsSummary(eqTo(validRegime), eqTo("XWM00000001770"))(any())
    ...
    "returns 400 when InvalidRegimeError" in new Setup {
      ...
      val req = FakeRequest(GET, "/gambling/repayment-summary/INVALID_REGIME/XWM00000001770")
      val res: Future[Result] = controller.getRepaymentsSummary(" ", " ")(req)
      ...
      verify(mockService).getRepaymentsSummary(eqTo(" "), eqTo(" "))(any())
    }
    ...
    "returns 400 when InvalidRegNumber" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/repayment-summary/$validRegime/InvalidRegNo")
      val res: Future[Result] = controller.getRepaymentsSummary(" ", " ")(req)
    ...
    "returns 500 when UnexpectedError" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/repayment-summary/$validRegime/ERR00001770")
      val res: Future[Result] = controller.getRepaymentsSummary(validRegime, "ERR00001770")(req)
