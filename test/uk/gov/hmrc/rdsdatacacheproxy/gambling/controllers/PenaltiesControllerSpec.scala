...
class PenaltiesControllerSpec extends SpecBase with MockitoSugar {
  ...
  "PenaltiesController#getPenalties" - {
    ...
    "returns 200 when service succeeds" in new Setup {
      ...
      when(mockService.getPenalties(eqTo(validRegime), eqTo("XWM00000001770"), eqTo(1), eqTo(10))(any()))
        .thenReturn(Future.successful(Right(validResponsePenalties)))

      val req = FakeRequest(GET, s"/gambling/penalties/$validRegime/XWM00000001770?pageNo=1&pageSize=10")
      val res: Future[Result] = controller.getPenalties(validRegime, "XWM00000001770", 1, 10)(req)

      status(res) mustBe OK
      contentType(res) mustBe Some(JSON)
      contentAsJson(res) mustBe Json.toJson(validResponsePenalties)

      verify(mockService).getPenalties(eqTo(validRegime), eqTo("XWM00000001770"), eqTo(1), eqTo(10))(any())
    ...
    "returns 400 when InvalidRegimeError" in new Setup {
      ...
      val req = FakeRequest(GET, "/gambling/penalties/INVALID_REGIME/XWM00000001770")
      val res: Future[Result] = controller.getPenalties(" ", " ", 1, 10)(req)
      ...
      verify(mockService).getPenalties(eqTo(" "), eqTo(" "), eqTo(1), eqTo(10))(any())
    }
    ...
    "returns 400 when InvalidRegNumber" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/penalties/$validRegime/InvalidRegNo")
      val res: Future[Result] = controller.getPenalties(" ", " ", 1, 10)(req)
    ...
    "returns 500 when UnexpectedError" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/penalties/$validRegime/ERR00001770")
      val res: Future[Result] = controller.getPenalties(validRegime, "ERR00001770", 1, 10)(req)
      ...
      verify(mockService).getPenalties(eqTo(validRegime), eqTo("ERR00001770"), eqTo(1), eqTo(10))(any())
    }
  }
}
