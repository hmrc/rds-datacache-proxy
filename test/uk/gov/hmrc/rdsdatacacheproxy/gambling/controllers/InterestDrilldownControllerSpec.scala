...
class InterestDrilldownControllerSpec extends SpecBase with MockitoSugar {
  ...
  private trait Setup {
    ...
    val controller = new InterestDrilldownController(fakeAuthAction, fakeAgentAuthAction, mockService, cc)
  }

  "InterestDrilldownController#getInterestDrilldown" - {
    ...
    "returns 200 when service succeeds" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/interest-drilldown/$validRegime/XWM00000001770/INT001?pageNo=1&pageSize=10")
    ...
    "returns 400 when InvalidRegimeError" in new Setup {
      ...
      verify(mockService).getInterestDrilldown(eqTo(" "), eqTo(" "), eqTo("INT001"), eqTo(1), eqTo(10))(any())
    }
    ...
    "returns 400 when InvalidRegNumber" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/interest-drilldown/$validRegime/InvalidRegNo/INT001")
      val res: Future[Result] = controller.getInterestDrilldown(" ", " ", "INT001", 1, 10)(req)
    ...
    "returns 500 when UnexpectedError" in new Setup {
      ...
      val req = FakeRequest(GET, s"/gambling/interest-drilldown/$validRegime/ERR00001770/INT001")
      ...
      verify(mockService).getInterestDrilldown(eqTo(validRegime), eqTo("ERR00001770"), eqTo("INT001"), eqTo(1), eqTo(10))(any())
    }
  }
}
