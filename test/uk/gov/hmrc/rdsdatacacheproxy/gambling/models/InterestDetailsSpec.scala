...
class InterestDetailsSpec extends AnyWordSpec with Matchers {

  "InterestDetails JSON format" should {

    "serialize to JSON correctly" in {
      val json = Json.toJson(validResponseInterestDetails)
      ...
      val items = (json \ "items").as[Seq[InterestDetailItem]]
      items.size mustBe 3
      ...
      (item2 \ "periodStartDate").as[String] mustBe "2014-10-01"
      (item2 \ "periodEndDate").as[String] mustBe "2014-12-31"
      ...
      (item3 \ "periodStartDate").as[String] mustBe "2013-04-01"
    ...
    "deserialize from JSON correctly" in {
      ...
      val result: JsResult[InterestDetails] = json.validate[InterestDetails]

      result mustBe JsSuccess(validResponseInterestDetails)
    }

    "round-trip write then read should return same object" in {
      val json = Json.toJson(validResponseInterestDetails)
      val parsed = json.as[InterestDetails]
      parsed mustBe validResponseInterestDetails
    }
    ...
    "fail to deserialize when required fields are missing" in {
      ...
      val result = json.validate[InterestDetails]
