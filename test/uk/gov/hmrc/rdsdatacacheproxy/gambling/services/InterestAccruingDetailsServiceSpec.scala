...
final class InterestAccruingDetailsServiceSpec extends SpecBase {

  private val repository = mock[InterestAccruingDetailsDataSource]
  private val service = new InterestAccruingDetailsService(repository)
  ...
  "InterestAccruingDetailsService#getInterestAccruingDetails" - {

    "return validResponseInterestAccruingDetails when repository succeeds AND normalise input (trim + uppercase) before calling repository" in {
      when(repository.getInterestAccruingDetails(eqTo(validRegime), eqTo(normalisedRegNumber), eqTo(1), eqTo(10)))
        .thenReturn(Future.successful(validResponseInterestAccruingDetails))

      val result = service.getInterestAccruingDetails(validRegime.toString, lowercaseRegNumber, 1, 10).futureValue

      result mustBe Right(validResponseInterestAccruingDetails)
      verify(repository).getInterestAccruingDetails(eqTo(validRegime), eqTo(normalisedRegNumber), eqTo(1), eqTo(10))
      verifyNoMoreInteractions(repository)
    }

    "return InvalidRegimeError and not call repository when Regime input is invalid" in {
      val result = service.getInterestAccruingDetails("INVALID", lowercaseRegNumber, 1, 10).futureValue
    ...
    "return InvalidRegNumber and not call repository when RegNumber input is invalid" in {
      ...
      val result = service.getInterestAccruingDetails(validRegime.toString, invalidRegNumber, 1, 10).futureValue
