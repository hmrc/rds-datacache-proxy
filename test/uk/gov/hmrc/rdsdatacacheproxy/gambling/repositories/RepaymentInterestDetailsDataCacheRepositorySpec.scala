...
import uk.gov.hmrc.rdsdatacacheproxy.shared.utils.GamblingTestUtil.validResponseRepaymentInterestDetailsSmall

import java.sql.{CallableStatement, Connection, Date, ResultSet}
import java.time.LocalDate
import scala.concurrent.ExecutionContext.Implicits.global

class RepaymentInterestDetailsDataCacheRepositorySpec extends AnyWordSpec with Matchers with BeforeAndAfter {

  private var mgdDb: MGDDatabase = _
  private var gtrDb: GTRDatabase = _
  var repository: RepaymentInterestDetailsDataCacheRepository = _
  ...
  var repaymentInterestDetailsRs: ResultSet = _
  ...
  before {
    ...
    repaymentInterestDetailsRs = mock(classOf[ResultSet])
    ...
    when(mgdMockConnection.prepareCall("{ call MGD_LNP_PK.getMGDRepaymentInterestDetails(?, ?, ?, ?, ?, ?, ?, ?) }")).thenReturn(mockCsMgd)
    when(gtrMockConnection.prepareCall("{ call GTR_LNP_PK.getGTRRepaymentInterestDetails(?, ?, ?, ?, ?, ?, ?, ?) }")).thenReturn(mockCsGtr)

    repository = new RepaymentInterestDetailsDataCacheRepository(
      mgdDb = mgdDb,
      gtrDb = gtrDb
    )
  }

  "getRepaymentInterestDetails" should {
    "return RepaymentInterestDetails for MGD regime when stored procedure returns data" in {
      ...
      when(mockCsMgd.getObject(8)).thenReturn(repaymentInterestDetailsRs)

      when(repaymentInterestDetailsRs.next()).thenReturn(true, false)

      when(repaymentInterestDetailsRs.getInt("p_desc_code")).thenReturn(2740)
      when(repaymentInterestDetailsRs.getObject("p_amount")).thenReturn(java.math.BigDecimal.valueOf(-45.00))
      when(repaymentInterestDetailsRs.getString("p_interest_id")).thenReturn("SAFE-CHG-00001")
      when(repaymentInterestDetailsRs.getDate("p_period_start")).thenReturn(Date.valueOf("2014-07-22"))
      when(repaymentInterestDetailsRs.getDate("p_period_end")).thenReturn(Date.valueOf("2014-10-31"))

      val result = repository.getRepaymentInterestDetails(Regime.MGD, regNumber, 1, 10).futureValue

      result            shouldBe validResponseRepaymentInterestDetailsSmall
      ...
      verify(repaymentInterestDetailsRs, times(2)).next()
      verify(repaymentInterestDetailsRs).getInt("p_desc_code")
      verify(repaymentInterestDetailsRs).getObject("p_amount")
      verify(repaymentInterestDetailsRs).getString("p_interest_id")
      verify(repaymentInterestDetailsRs).getDate("p_period_start")
      verify(repaymentInterestDetailsRs).getDate("p_period_end")

      verify(repaymentInterestDetailsRs).close()
      verify(mockCsMgd).close()
    }

    "return empty RepaymentInterestDetails when regNumber is null" in {
      ...
      verify(mockCsMgd).getObject(7)
      verify(mockCsMgd).getObject(8)
      verify(repaymentInterestDetailsRs, times(0)).next()
      verify(repaymentInterestDetailsRs, times(0)).getInt("p_desc_code")
      verify(repaymentInterestDetailsRs, times(0)).close()
      verify(mockCsMgd).close()
    }

    "return Empty List when RepaymentInterestDetails result set is empty" in {
      ...
      when(repaymentInterestDetailsRs.next()).thenReturn(false)
    ...
    Regime.values.toList.filterNot(_ == Regime.MGD).foreach { regime =>
      s"return RepaymentInterestDetails for $regime regime when stored procedure returns data" in {
        ...
        when(mockCsGtr.getObject(8)).thenReturn(repaymentInterestDetailsRs)
        ...
        val result = repository.getRepaymentInterestDetails(regime, regNumber, 1, 10).futureValue
