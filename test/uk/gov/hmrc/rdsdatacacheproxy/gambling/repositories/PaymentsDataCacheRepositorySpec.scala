...
class PaymentsDataCacheRepositorySpec extends AnyWordSpec with Matchers with BeforeAndAfter {
  ...
  before {
    ...
    when(mgdMockConnection.prepareCall("{ call MGD_LNP_PK.getMGDPayments(?, ?, ?, ?, ?, ?, ?, ?) }")).thenReturn(mockCsMgd)
    when(gtrMockConnection.prepareCall("{ call GTR_LNP_PK.getGTRPayments(?, ?, ?, ?, ?, ?, ?, ?) }")).thenReturn(mockCsGtr)

    repository = new PaymentsDataCacheRepository(
      mgdDb = mgdDb,
      gtrDb = gtrDb
    )
  }
  ...
  "getPayments" should {
    "return Payments for MGD regime when stored procedure returns data" in {
      ...
      result            shouldBe validResponsePaymentsSmall
      ...
      verify(paymentsRs, times(2)).next()
      verify(paymentsRs).getDate("p_transaction_date")
      verify(paymentsRs).getString("p_desc_code")
      verify(paymentsRs).getObject("p_amount")

      verify(paymentsRs).close()
      verify(mockCsMgd).close()
    }

    "return empty Payments when regNumber is null" in {
      val regNumber: Null = null
      when(mockCsMgd.getDate(2)).thenReturn(null)
      val result = repository.getPayments(Regime.MGD, regNumber, 1, 10).futureValue

      result       shouldBe Payments(None, None, BigDecimal(0), 0, List())
      ...
      verify(mockCsMgd).getObject(7)
      verify(mockCsMgd).getObject(8)
      verify(paymentsRs, times(0)).next()
      verify(paymentsRs, times(0)).getDate("p_transaction_date")
      verify(paymentsRs, times(0)).close()
      verify(mockCsMgd).close()
    }

    "return Empty List when Payments result set is empty" in {
      ...
      when(paymentsRs.next()).thenReturn(false)

      val result = repository.getPayments(Regime.MGD, regNumber, 1, 10).futureValue

      result shouldBe Payments(Some(LocalDate.of(2016, 2, 29)), Some(LocalDate.of(2017, 6, 15)), -301.56, 0, List())
    ...
    Regime.values.toList.filterNot(_ == Regime.MGD).foreach { regime =>
      s"return Payments for $regime regime when stored procedure returns data" in {
        ...
        when(mockCsGtr.getObject(8)).thenReturn(paymentsRs)
