...
class PaymentsCtDataCacheRepositorySpec extends AnyWordSpec with Matchers with BeforeAndAfter {
  ...
  "getPayments" should {
    "return a list of Payment Transactions when stored procedure returns data" in {
      ...
      when(mockResultSet.getBigDecimal("amount")).thenReturn(BigDecimal(123.44).bigDecimal)
      when(mockResultSet.getString("payment_type")).thenReturn("A")
      when(mockResultSet.getDate("effective_date_of_payment")).thenReturn(Date.valueOf("2026-01-01"))
    ...
    "return a list of multiple Payment Transactions when stored procedure returns data" in {
      ...
      when(mockResultSet.getBigDecimal("amount")).thenReturn(BigDecimal(123.44).bigDecimal, BigDecimal(456.77).bigDecimal)
      when(mockResultSet.getString("payment_type")).thenReturn("A", "D")
      when(mockResultSet.getDate("effective_date_of_payment")).thenReturn(Date.valueOf("2026-01-01"), Date.valueOf("2026-02-01"))
    ...
    "return an empty list of Payment Transactions when stored procedure returns no data" in {
