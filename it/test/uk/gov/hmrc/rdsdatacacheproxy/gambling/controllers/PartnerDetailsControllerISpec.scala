...
import uk.gov.hmrc.rdsdatacacheproxy.gambling.repositories.PartnerDetailsDataSource
import uk.gov.hmrc.rdsdatacacheproxy.gambling.stub.PartnerDetailsStubData
...
class PartnerDetailsControllerISpec extends AnyWordSpec with Matchers with ScalaFutures with IntegrationPatience with ApplicationWithWiremock {
  ...
  class PartnerDetailsRdsStub extends PartnerDetailsDataSource {
    override def getPartnerDetails(regime: Regime, regNumber: String): Future[PartnerDetails] =
      Future {
        PartnerDetailsStubData.getPartnerDetailsData(regNumber)
      }
  }
  ...
  override lazy val app: Application =
    new GuiceApplicationBuilder()
      ...
      .overrides(
        bind[PartnerDetailsDataSource].toInstance(new PartnerDetailsRdsStub)
      )
      .build()

  private final val endpoint = "/gambling/partner-details"
  private final val MGD = "mgd"

  "GET /gambling/partner-details (stubbed repo, no DB)" should {
    ...
    "return 200 with correct ReturnsPartnerDetailsData" in {
      ...
      response.json.as[PartnerDetails] mustBe getPartnerDetailsData("XYZ00000000000")
    }
    ...
    "return consistent results across multiple calls" in {
      AuthStub.authorised()
      val res1 = get(s"$endpoint/$MGD/XYZ00000000000").futureValue
      val res2 = get(s"$endpoint/$MGD/XYZ00000000000").futureValue
