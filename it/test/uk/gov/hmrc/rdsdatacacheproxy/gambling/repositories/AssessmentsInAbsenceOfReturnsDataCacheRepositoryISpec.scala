...
class AssessmentsInAbsenceOfReturnsDataCacheRepositoryISpec extends AnyWordSpec with Matchers with ScalaFutures with IntegrationPatience with GuiceOneAppPerSuite {
  ...
  "getAssessmentsInAbsenceOfReturns(stubbed repository)" should {
    ...
    "return correct AssessmentsInAbsenceOfReturnsData" in {
      val result = repository.getAssessmentsWithoutReturn(Regime.MGD, "XGM00003122200", 1, 10).futureValue
    ...
    "return consistent results across multiple calls" in {
      val result1 = repository.getAssessmentsWithoutReturn(Regime.MGD,"XGM00003122200", 1, 10).futureValue
      val result2 = repository.getAssessmentsWithoutReturn(Regime.MGD,"XGM00003122200", 1, 10).futureValue
