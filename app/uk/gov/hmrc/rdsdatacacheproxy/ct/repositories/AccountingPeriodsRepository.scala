  ...
  private def readAccountingPeriods(rs: ResultSet): List[AccountingPeriodsInfo] = {
    ...
    while (rs.next()) {
      buffer += AccountingPeriodsInfo(
        ...
        taxChargePresent       = rs.getString("tax_charge_present"),
        clericalIntSig         = rs.getString("clerical_int_sig"),
        creditDebitInterestInd = rs.getString("credit_debit_interest_ind"),
