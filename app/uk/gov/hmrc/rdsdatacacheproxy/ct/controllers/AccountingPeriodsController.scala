...
class AccountingPeriodsController @Inject() (
  ...
  def getAccountPeriods(taxRef: Long): Action[AnyContent] = authorise.async { implicit request =>
    accountingPeriodsService
      .getAccountPeriods(taxRef)
      .map { accountingPeriods =>
        Ok(Json.toJson(accountingPeriods))
      }
      .recover { case ex: Exception =>
        logger.error("Error while retrieving accounting periods list", ex)
        InternalServerError(Json.obj("error" -> "Failed to retrieve accounting periods list"))
      }
  }
}
