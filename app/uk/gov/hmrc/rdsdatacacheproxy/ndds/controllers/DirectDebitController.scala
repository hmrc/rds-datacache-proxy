...
import uk.gov.hmrc.rdsdatacacheproxy.ndds.models.requests.{GenerateDdiRefRequest, WorkingDaysOffsetRequest}
...
class DirectDebitController @Inject() (
  ...
  def generateDDIReference(): Action[GenerateDdiRefRequest] =
    authorise.async(parse.json[GenerateDdiRefRequest]) { implicit request =>
      directDebitService
        .getDDIReference(request.body.paymentReference, request.credentialId, request.sessionId.value)
